      ...
      <div className="mx-auto max-w-[1440px] px-6 py-20 md:px-16 md:py-28">
        <div className="grid items-center gap-12 md:grid-cols-2">
          ...
          <div className="order-1 md:order-2 md:text-right">
            ...
            <h2 className="mt-6 text-[36px] leading-[1.08] sm:text-[48px] lg:text-[60px]">
              Comprehensive
              <br />
        ...
        <div className="mt-16 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {services.map((s) => (
            <article key={s.n} className="rounded-lg border border-primary p-6">
              <p className="text-[11px] italic">Service</p>
              <p className="mt-3 text-[32px] leading-none">{s.n}</p>
              <h3 className="mt-6 border-b border-primary pb-3 text-[17px]">{s.title}</h3>
              <p className="mt-4 text-[13px] italic leading-[1.6]">{s.desc}</p>
              <ul className="mt-5 space-y-1 text-[12px] italic">
                {s.points.map((p) => (
                  <li key={p}>·{p}</li>
                ))}
              </ul>
